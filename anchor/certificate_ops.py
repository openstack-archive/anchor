...
def _run_validator(validator_step, pecan_args):
    """Parse the validator step, call the validator, and return result.

       :param validator_step: User config as a dict of dicts
       :param args: additional arguments to pass to the validator function
       :return: True on success, else False
    """
    # TODO(hyakuhei): Consider adding dd some validation checks
    # (though the config validator should have already done this)
    ...
    if not hasattr(validators, function_name):
        logger.error(
            "_run_validator: no validator method {}".format(validator_step)
        )
        return False

    # careful to not modify the master copy of args with local params
    new_kwargs = pecan_args.copy()
    new_kwargs.update(params)

    # perform the actual check
    try:
        getattr(validators, function_name)(**new_kwargs)
