...
def parse_csr(csr, encoding):
    try:
        if encoding != 'pem' or csr is None:
            logger.error("Parsing CSR failed due to non-pem encoding type or null CSR.")
            return None
...
def validate_csr(auth_result, csr, request):
    ...
    try:
        ...
        raise validators.ValidationError("Incorrect M2Crypto library version, cannot perform csr.get_extensions")
