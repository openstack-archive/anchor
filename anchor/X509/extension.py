...
def modifies_ext_value(f):
    def ext_value_filled(self, *args, **kwargs):
        ...
        # since some elements like NamedValue are pure value types, there is no interface to modify them and new versions have to be returned
...
class X509Extension(object):
    ...
    def _get_default_value(cls):
        # if there are any non-optional fields, this needs to be defined in the class
        return cls.spec()

    def __str__(self):
        return "%s %s" % (self.get_name(), self.get_value_as_str())

    def get_value_as_str():
        return "<unknown>"
    ...
    def _set_value(self, value):
        ...
        self._ext['extnValue'] = encoder.encode(rfc2459.univ.OctetString(encoder.encode(value)))
...
class X509ExtensionBasicConstraints(X509Extension):
    ...
    def __str__(self):
        return "basicConstraints: CA: %s, pathLen: %s" % (str(self.get_ca()).upper(), self.get_path_len_constraint())
...
class X509ExtensionKeyUsage(X509Extension):
    ...
    def get_all_usages(self, ext_value=None):
        return [self.inv_fields[i] for i, enabled in enumerate(ext_value) if enabled]
...
def _asn1_to_netaddr(octet_string):
...
class X509ExtensionSubjectAltName(X509Extension):
    ...
    def get_dns_ids(self, ext_value=None):
        return [name.getComponent().asOctets() for name in ext_value if name.getName() == 'dNSName']
    ...
    def get_ips(self, ext_value=None):
        ...
        for name in ext_value:
            if name.getName() != 'iPAddress':
                continue
            ips.append(_asn1_to_netaddr(name.getComponent()))
        return ips
    ...
    def add_ip(self, ip, ext_value=None):
        ...
        new_pos = len(ext_value)
        ext_value[new_pos] = None
        ext_value[new_pos]['iPAddress'] = _netaddr_to_asn1(ip)
        return ext_value
