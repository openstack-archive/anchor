...
# TODO(stan): The notBefore / notAfter times are a lie here. Openssl's X509
# returns ASN1_TIME, not ASN1_UTCTIME from notBefore/notAfter functions.
# Unfortunately cryptography package does not support ASN1_TIME_set and other
# functions in version 0.9. Cloning and fixing the whole binding just to add
# those two functions would be too much work, so for now, we're pretending the
# UTCTIME functions will work as expected and not crash. The offset is
# corrected manually using the calendar module and time.timezone values.
#
