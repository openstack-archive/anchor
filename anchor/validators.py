...
def common_name(csr, allowed_domains=[], **kwargs):
    ...
    if len(CNs) == 0 and len(csr.get_subject()) == 0:
...
def key_usage(csr=None, allowed_usage=None, **kwargs):
    ...
    allowed = set(extension.LONG_KEY_USAGE_NAMES.get(x, x) for x in allowed_usage)
    denied = set()
...
def ca_status(csr=None, ca_requested=False, **kwargs):
    ...
    for ext in (csr.get_extensions() or []):
        if isinstance(ext, extension.X509ExtensionBasicConstraints):
            if ext.get_ca():
                if not ca_requested:
                    raise ValidationError("CA status requested, but not allowed")
