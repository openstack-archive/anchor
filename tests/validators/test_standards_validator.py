...
class TestExtensionEmptySanDns(unittest.TestCase):
    def test_san_ok(self):
        csr = signing_request.X509Csr()
        ext = extension.X509ExtensionSubjectAltName()
        ext.add_dns_id('example.com')
        csr.add_extension(ext)
        validators_standards._no_empty_san_dns(csr)
...
class TestExtensionCriticalFlags(unittest.TestCase):
    ...
    def tes_with_subject_san_not_critical(self):
        csr = signing_request.X509Csr()
        subject = name.X509Name()
        subject.add_name_entry(x509_name.OID_commonName, "example.com")
